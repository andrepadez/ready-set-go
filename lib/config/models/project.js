...
Project.prototype.update = function () {
  ...
  return fs.existsAsync(project.pkgPath)
    .then(function (exists) {
      ...
      return fs.readFileAsync(project.pkgPath)
        ...
        .catch((err) => {
          log.error('package.json', project.name || project.pkgPath, err.message)
          if(!project.name){
            log.error('gaston', 'please fix package.json and relaunch gastond')
            process.exit(1)
          }
        })
    })
...
Project.prototype.save = function () {
  ...
  console.log()
  return fs.mkdirp(path.dirname(project.projectFile))
    .then(() => fs.writeFileAsync(project.projectFile, data))
    .then(function () {
      return project
    })
}
...
Project.prototype.unregister = function () {
  ...
  return project.stop()
    .then(function () {
      ...
      return fs.existsAsync(project.projectFile)
        .then((exists) => {
          if(exists){
