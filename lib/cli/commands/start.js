...
module.exports = function start (args) {
  ...
  log.info('ready-set-go', 'launching project:', pkg.name)
  ...
  return Config.init(true)
    .then(getPort)
    .then(Daemon.start)
    .then(() => { return args.p || args.port || (pkg.gaston && pkg.gaston.port) || getPort() })
    .then((port) => {
      ...
      Config.startProject(project)
        .then((project) => {
          ...
          log.info('ready-set-go', 'running on', url)
